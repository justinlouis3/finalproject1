      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-4">
          <button
            ...
            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200"
          >
            Submit
          </button>
        </div>
...
function MessyComponent() {
  ...
  // Filters the items array to only include items matching the specified category (case-insensitive)
  const filterByCategory = (items: Item[], category: string) => {
    if (category === '') return items
    return items.filter(item => item.category.toLowerCase() === category.toLowerCase())
  }

  // Helper function to filter only in-stock items
  const filterInStockItems = (items: Item[]) => {
    return items.filter(item => item.inStock)
  }

  // Helper function to sort items
  const sortItems = (items: Item[], sortBy: string) => {
  ...
  const calculateDiscount = (price: number) => {
    return price > 2 ? price * 0.9 : price
  }

  // Helper function to add pricing info to items
  const addPricingInfo = (items: Item[]) => {
  ...
  // Process items using helper functions
